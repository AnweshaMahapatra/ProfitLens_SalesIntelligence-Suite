...
**Why:** First step before any analysis — confirm the structure of customer and product dimension tables. This revealed the `customer_type` field (Brick & Mortar vs E-Commerce) that later drives the channel split donut chart, and flagged that product codes follow a `ProdXXX` naming convention with a significant volume of nulls.
...
**Why:** Establishes a control figure. After all cleaning and transformation in Power Query, the final row count in Power BI was compared against this number to confirm nothing was accidentally dropped during the ETL process.
...
**Why:** Mark001 is Delhi NCR — the largest market by revenue. This query identified which products were driving Delhi NCR's ₹520M contribution and first surfaced the blank product code issue. A large number of transactions had null `product_code` values, which later became the "(Blank)" category showing ₹0.47bn on the dashboard — a data quality flag requiring resolution at the source system.
...
**Why:** This join is the structural foundation for all time-intelligence in the model. By joining the fact table to the date dimension on `order_date = date`, year and month filters become possible. The 2020 filter here validated the record set that feeds the entire Performance Analysis page (₹142M revenue, 350K units, ₹2.1M profit margin).
...
**Why:** Validated the 2020 revenue total (₹142M) before the Power BI Revenue measure was built. Running this in SQL first ensured the DAX result would match — if they disagreed, it pointed to either a currency standardisation gap or a filtering issue in Power Query.
...
**Why:** The most granular validation query — filtering by year, month, and market simultaneously. This confirmed that the data model could support multi-dimensional slicing (the year/month/market filter interaction visible on every dashboard page) and validated the Delhi NCR January 2020 figure used in the Performance Analysis trend.
...
**Why `new_sales_amount` and not `sales_amount`:** The raw `sales_amount` field contained values in mixed currencies — some transactions were recorded in USD. During Power Query cleaning, a conditional column `new_sales_amount` was created to convert all values to INR. Every revenue calculation in the model references this cleaned column. Using the raw field would produce an overstated and inaccurate ₹985M figure.
...
Each market's or customer's share of total revenue. The `CALCULATE(..., ALL(...))` pattern removes the current filter context from the three dimension tables so the denominator always equals the full dataset total — regardless of which market or customer row is being evaluated. Without `ALL()`, the denominator would equal the numerator in every row and every contribution % would show 100%.
...
Prior year revenue for the equivalent time period. `SAMEPERIODLASTYEAR()` shifts the date filter context back exactly one year using the `sales date` dimension table. This measure requires a properly structured date dimension — a continuous date table with no gaps — to function correctly.
